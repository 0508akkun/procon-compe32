...
Commands pointn::beamsearch7(const FFS field, const int timelimit_ms)
{
    std::chrono::system_clock::time_point start;
    start = std::chrono::system_clock::now();
    int margin_ms = 1800; //猶予時間
    //if(field.turn == 1) margin_ms = 1500;
    ...
    {   
        ...
        std::vector<std::priority_queue<State7>> States(depth+2);
        ...
        while(time < oneLimit*(i+1))
        {
            for(int d=0;d<depth;d++)
            {
                ...
                State7 now = States[d].top(); States[d].pop();
...
std::vector<Action> pointn::solve_beam7(const Field& field, const Match& match)
{
    ...
    Commands commands = beamsearch7(toFFS(field, match.turns, team_index, 
