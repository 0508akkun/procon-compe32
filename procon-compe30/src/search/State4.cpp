...
void State4::calcScore()
{
    score = LeadScoreCalculate(field, 3, 1, 2, 2);
}

void State4::calcScores(const int d, const int left)
{
    // pointListの更新 
    ...
    pointList[d][7] = pointList[d][0]*3 + pointList[d][1]*1 - (pointList[d][2]*2 + pointList[d][3]*2);
    score = pointList[d][7];
    // 終盤は実得点で計算
    if(left <= 5) score = pointList[d][6];
}
