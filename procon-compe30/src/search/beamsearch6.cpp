...
Commands ngng::beamsearch6(const FFS field, const int timelimit_ms)
{
    ...
    {   
        ...
        std::vector<std::priority_queue<State6>> States(depth+2);
        ...
        while(time < oneLimit*(i+1))
        {
            for(int d=0;d<depth;d++)
            {
                ...
                State6 now = States[d].top(); States[d].pop();
...
std::vector<Action> ngng::solve_beam6(const Field& field, const Match& match)
{
    ...
    Commands commands = beamsearch6(toFFS(field, match.turns, team_index, 
