...
Commands pointn::beamsearch7_2(const FFS field, const int timelimit_ms)
{
    ...
    const int depth = std::min(3,field.maxturn-field.turn+1); //depth手先まで見る(相手は考えない)
    ...
    {   
        ...
        std::vector<std::priority_queue<State7_2>> States(depth+2);
        ...
        while(time < oneLimit*(i+1))
        {
            for(int d=0;d<depth;d++)
            {
                ...
                State7_2 now = States[d].top(); States[d].pop();
...
std::vector<Action> pointn::solve_beam7_2(const Field& field, const Match& match)
{
    ...
    Commands commands = beamsearch7_2(toFFS(field, match.turns, team_index, 
