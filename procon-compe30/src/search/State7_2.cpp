...
void State7_2::calcScore()
{
    score = LeadScoreCalculate(field, 3, 1, 2, 2);
}

void State7_2::calcScores(const int d, const int left)
{
    // pointListの更新 
...
State7_2 pointn::transition(const State7_2& state, const int id, const Point dir,
                  ...
                  const int d, const int left)
{
    ...
    // タイル点数低すぎ //停留措置
    if(next.field.points[targetPos.y][targetPos.x] < -5){
        Command tmpc;
        next.via[d] = std::make_pair(tmpc,std::make_pair(here,here));
        return next;
    }
