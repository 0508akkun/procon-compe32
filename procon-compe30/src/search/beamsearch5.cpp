...
Commands ngng::beamsearch5(const FFS field, const int timelimit_ms)
{
    ...
    {   
        ...
        std::vector<std::priority_queue<State5>> States(depth+2);
        ...
        while(time < oneLimit*(i+1))
        {
            for(int d=0;d<depth;d++)
            {
                ...
                State5 now = States[d].top(); States[d].pop();
...
std::vector<Action> ngng::solve_beam5(const Field& field, const Match& match)
{
    ...
    Commands commands = beamsearch5(toFFS(field, match.turns, team_index, 
