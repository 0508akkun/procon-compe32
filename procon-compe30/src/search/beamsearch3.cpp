...
Commands beamsearch3(const FFS field, const int timelimit_ms)
{
    ...
    {   
        ...
        std::vector<std::priority_queue<State3>> States(depth+2);
        ...
        while(time < oneLimit*(i+1))
        {
            for(int d=0;d<depth;d++)
            {
                ...
                State3 now = States[d].top(); States[d].pop();
...
std::vector<Action> solve_beam3(const Field& field, const Match& match)
{
    ...
    Commands commands = beamsearch3(toFFS(field, match.turns, team_index, 
